...
# https://neetcode.io/solutions/split-array-largest-sum
# 04:09
class Solution:
    def splitArray(self, nums: list[int], k: int) -> int:
        ...
        # all the while you're tracking the max value seen so far
        
        self.maxChunks = float("inf")
        self.explore(nums, 0, k, [], set())
        
        return self.maxChunks
    
    def explore(self, nums, currIdx, chunksLeft, maxSoFar, memo):
        mitem = (currIdx, chunksLeft)
        if mitem in memo:
            return
        ...
        for idx in range(currIdx, endRange):
            ...
            self.explore(nums, idx + 1, chunksLeft - 1, maxSoFar, memo)
