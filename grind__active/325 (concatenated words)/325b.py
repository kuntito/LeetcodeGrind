...
# this situation works but TLE
# i want to memoize repeated work

# first thought that comes to mind is to store
# the results of each word exploration

# but i encountered a flaw in my reasoning
# consider ["pep", "si", "pepsi"]
# memo = {"pep": False, "si": False}

# by the time i explore "pepsi"
# and get to the substring "pep"
# i'd check if the substring is in all words
# if it is, i'd repeat the process on the rest of the word

# problem is, the rest of the word is "si"
# and according to memo, this returns False

# it's true that "si" in isolation is not a concatenated word
# but "si" is a word and should return True since it's part of the concatenation for "pepsi"


# i tweaked the memo check to make return memo[word] or word in self.allWords
# this way if the word has been explored and returned False, we'd return True if the word exists in all words
# but it fails a test case and i'm not sure why
# i'd rewrite the memoized code and take it from there

# TODO i'm not sure why the last test case has single characters in the result.
class Solution:
    def findAllConcatenatedWordsInADict(self, words: List[str]) -> List[str]:
        ...
        res = []
        for w in words:
            isConcat = self.explore(0, w)
            if isConcat:
                res.append(w)
            self.memo[w] = isConcat
    ...
    def explore(self, count, word):
        ...
        if word in self.memo:
            return self.memo[word] or (word in self.allWords and count >= 1)
        ...
        for idx in range(dim):
            ...
            if substr in self.allWords:
                resOfWord = word[endIdx:]
                isConcat = self.explore(count, resOfWord)
                
                self.memo[resOfWord] = isConcat
                if isConcat:
                    return True
